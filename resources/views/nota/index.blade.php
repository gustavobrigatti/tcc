                ...
                <div class="header">
                    <h2>
                        @if(\Illuminate\Support\Facades\Auth::user()->role == 600 && !isset($_GET['al']))
                            ALUNOS
                        @elseif(\Illuminate\Support\Facades\Auth::user()->role == 200 && isset($_GET['t']) && !isset($_GET['al']))
                            ALUNOS
                        @else
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover dataTable" style="white-space: nowrap;">
                                ...
                                @forelse(\Illuminate\Support\Facades\Auth::user()->alunos as $aluno)
                                    <tr>
                                        <td>{{ $aluno->name }}</td>
                                        <td><a href="/nota?al={{ $aluno->hash_id }}" class="btn btn-primary btn-block waves-effect" >VISUALIZAR</a></td>
                                    </tr>
                                @empty
                                @endforelse
                                </tbody>
                    ...
                    @elseif(\Illuminate\Support\Facades\Auth::user()->role == 200 && isset($_GET['t']) && !isset($_GET['al']))
                        <div class="table-responsive">
                            <table class="table table-bordered table-striped table-hover dataTable" style="white-space: nowrap;">
                                ...
                                <tbody>
                                @forelse(\App\Models\Turma::where('id', \Vinkla\Hashids\Facades\Hashids::decode($_GET['t']))->first()->alunos as $aluno)
                                    <tr>
                                        <td>{{ $aluno->name }}</td>
                                        <td><a href="/nota?t={{ $_GET['t'] }}&al={{ $aluno->hash_id }}" class="btn btn-primary btn-block waves-effect" >VISUALIZAR</a></td>
                                    </tr>
                                @empty
                                @endforelse
                                </tbody>
                            ...
                            <table class="table table-bordered table-striped table-hover js-basic-example dataTable" style="white-space: nowrap;">
                                @if(isset($_GET['t']))
                                    ...
                                    @forelse($aulas as $aula)
                                        <tr>
                                            <td>{{ $aula->nome }}</td>
                                            @if(\Illuminate\Support\Facades\Auth::user()->role == 200 || \Illuminate\Support\Facades\Auth::user()->role == 600)
                                                <td><a href="/nota/{{ $turma->hash_id }}?a={{ $aula->hash_id }}&al={{ $_GET['al'] }}" class="btn btn-primary btn-block waves-effect" >VISUALIZAR</a></td>
                                            @else
                                    ...
                                    @forelse($turmas as $turma)
                                        <tr>
                                            ...
                                            @if(\Illuminate\Support\Facades\Auth::user()->role == 600)
                                                <td><a href="/nota?t={{ $turma->hash_id }}&al={{ $_GET['al'] }}" class="btn btn-primary btn-block waves-effect" >VISUALIZAR</a></td>
                                            @else
